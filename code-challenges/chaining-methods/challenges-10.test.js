...
const count = (target, input) => {
  // Solution code here...
 return input.reduce((acc, it) => [...acc,...it], []).filter((b) => {
   if(b === target){
     return true;
   }
 }).length;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, it) => [...acc,...it], []).reduce((a, b) => {
    return a + b   
  })
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(myArr => { 
    return myArr.filter(myMapArr => {
      if (typeof myMapArr === 'number' && myMapArr % 5 === 0){
        return true;
      }
    }).map(result => {return Math.pow(2, result)
  })
})
};
