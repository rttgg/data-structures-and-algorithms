...
const firstLetters = (arr) => {
  ...
  for (let i =0; i < arr.length; i++){
    myNewLetter.push(arr[i].charAt(0))
  }
  return myNewLetter;
};
...
const findHappiness = (arr) => {
  // Solution code here...
 return arr.filter(smilly => smilly.includes(':)'))
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(comma => 
    comma.substring(1,4) + comma.substring(6,9) + comma.substring(10,14))
...
const onlyOddChars = (str) => {
  // Solution code here...
 let odd = str.split('');
 let oddChars = odd.filter((val, idx) => {
   if(idx % 2 !== 0) {
     return true;
   }
 });
